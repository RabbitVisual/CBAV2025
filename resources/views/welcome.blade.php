...
<head>
    ...
    <title>{{ isset($configuracoes['meta_title']) && $configuracoes['meta_title']->valor ? $configuracoes['meta_title']->valor : (isset($configuracoes['igreja_nome']) && $configuracoes['igreja_nome']->valor ? $configuracoes['igreja_nome']->valor : 'Congregação Batista Avenida') }}</title>
    <meta name="description" content="{{ $configuracoes['meta_description']->valor ?? 'Uma igreja comprometida com o amor de Cristo' }}" />
    <meta name="keywords" content="{{ $configuracoes['meta_keywords']->valor ?? 'igreja, batista, comunidade, fé' }}" />
    ...
    <style>
        .gradient-bg {
            background: linear-gradient(135deg, {{ $configuracoes['cor_primaria']->valor ?? '#1e40af' }} 0%, {{ $configuracoes['cor_secundaria']->valor ?? '#3b82f6' }} 100%);
        }
        body {
        ...
        .accent-color { color: {{ $configuracoes['cor_destaque']->valor ?? '#10b981' }}; }
        
        .secondary-color { color: {{ $configuracoes['cor_secundaria']->valor ?? '#3b82f6' }}; }
        ...
        .gradient-text { background: linear-gradient(135deg, {{ $configuracoes['cor_primaria']->valor ?? '#1e40af' }}, {{ $configuracoes['cor_destaque']->valor ?? '#10b981' }}); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
...
<body class="bg-gray-50">
    <!-- Header Profissional -->
    <header class="bg-gradient-to-r from-blue-600 to-blue-800 hero-pattern sticky top-0 z-50 shadow-xl" style="background: linear-gradient(135deg, {{ $configuracoes['cor_primaria']->valor ?? '#1e40af' }} 0%, {{ $configuracoes['cor_secundaria']->valor ?? '#3b82f6' }} 100%);">
        <nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    @if(($configuracoes['header_logo_ativa']->valor ?? '1') == '1')
                        @if(isset($configuracoes['logo']) && $configuracoes['logo']->valor)
                            <img src="{{ Storage::url($configuracoes['logo']->valor) }}" alt="Logo" class="w-12 h-12 object-contain bg-white rounded-xl p-1 shadow-lg" onerror="this.onerror=null;this.src='{{ asset('img/logo.png') }}'">
                        @else
                    ...
                    @endif
                    <div>
                        @if(($configuracoes['header_nome_igreja_ativa']->valor ?? '1') == '1')
                            <h1 class="text-2xl font-bold text-white text-shadow">{{ $configuracoes['igreja_nome']->valor ?? 'Congregação Batista Avenida' }}</h1>
                        @endif
                        @if(($configuracoes['header_slogan_ativa']->valor ?? '1') == '1')
                            <p class="text-white text-opacity-90 text-sm font-medium">{{ $configuracoes['igreja_slogan']->valor ?? 'Uma igreja comprometida com o amor de Cristo' }}</p>
                        @endif
                    </div>
                </div>
                <!-- Menu Desktop - Apenas em telas médias e grandes -->
                <div class="hidden lg:flex items-center space-x-6">
                    @if(($configuracoes['header_menu_ativa']->valor ?? '1') == '1')
                        <a href="#sobre" class="text-white hover:text-blue-200 transition-all duration-300 font-medium hover:scale-105">{{ $configuracoes['header_link_sobre']->valor ?? 'Sobre' }}</a>
                        <a href="#ministerios" class="text-white hover:text-blue-200 transition-all duration-300 font-medium hover:scale-105">{{ $configuracoes['header_link_ministerios']->valor ?? 'Ministérios' }}</a>
                        <a href="#cultos" class="text-white hover:text-blue-200 transition-all duration-300 font-medium hover:scale-105">{{ $configuracoes['header_link_cultos']->valor ?? 'Cultos' }}</a>
                        <a href="#aniversariantes" class="text-white hover:text-blue-200 transition-all duration-300 font-medium hover:scale-105">{{ $configuracoes['header_link_aniversariantes']->valor ?? 'Aniversariantes' }}</a>
                        <a href="{{ route('public.eventos.index') }}" class="text-white hover:text-blue-200 transition-all duration-300 font-medium hover:scale-105">{{ $configuracoes['header_link_eventos']->valor ?? 'Eventos' }}</a>
                        <a href="{{ route('doacao.index') }}" class="text-white hover:text-blue-200 transition-all duration-300 font-medium hover:scale-105">{{ $configuracoes['header_link_doacao']->valor ?? 'Doação' }}</a>
                        <a href="#contato" class="text-white hover:text-blue-200 transition-all duration-300 font-medium hover:scale-105">{{ $configuracoes['header_link_contato']->valor ?? 'Contato' }}</a>
                    @endif
                    ...
                    @auth
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <div class="flex space-x-2">
                                @if(auth()->user()->hasRole('Membro'))
                                    <a href="{{ route('member.dashboard') }}" class="glass-effect text-white px-4 py-2 rounded-lg hover:bg-white hover:text-blue-600 transition-all duration-300 text-sm font-medium">
                                        <i class="fas fa-user mr-1"></i>{{ $configuracoes['header_texto_area_membro']->valor ?? 'Área do Membro' }}
                                    </a>
                                @elseif(auth()->user()->hasAnyRole(['Super Admin', 'Pastor', 'Líder']))
                                    <a href="{{ route('admin.dashboard') }}" class="glass-effect text-white px-4 py-2 rounded-lg hover:bg-white hover:text-blue-600 transition-all duration-300 text-sm font-medium">
                                        <i class="fas fa-cog mr-1"></i>Painel Admin
                                    </a>
                                @elseif(auth()->user()->hasRole('Tesoureiro'))
                    ...
                    @else
                        <!-- Usuário Não Logado -->
                        <a href="{{ route('login') }}" class="glass-effect text-white px-6 py-3 rounded-xl hover:bg-white hover:text-blue-600 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl">
                            <i class="fas fa-sign-in-alt mr-2"></i>{{ $configuracoes['header_texto_area_membro']->valor ?? 'Área do Membro' }}
                        </a>
                    @endauth
                </div>
            ...
            <div id="mobileMenu" class="lg:hidden hidden mt-4 pb-4">
                <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/20 shadow-xl">
                    <div class="flex flex-col space-y-3">
                        @if(($configuracoes['header_menu_ativa']->valor ?? '1') == '1')
                            <a href="#sobre" class="text-white hover:text-blue-200 transition-colors font-medium py-2 px-3 rounded-lg hover:bg-white/10">{{ $configuracoes['header_link_sobre']->valor ?? 'Sobre' }}</a>
                            <a href="#ministerios" class="text-white hover:text-blue-200 transition-colors font-medium py-2 px-3 rounded-lg hover:bg-white/10">{{ $configuracoes['header_link_ministerios']->valor ?? 'Ministérios' }}</a>
                            <a href="#cultos" class="text-white hover:text-blue-200 transition-colors font-medium py-2 px-3 rounded-lg hover:bg-white/10">{{ $configuracoes['header_link_cultos']->valor ?? 'Cultos' }}</a>
                            <a href="#aniversariantes" class="text-white hover:text-blue-200 transition-colors font-medium py-2 px-3 rounded-lg hover:bg-white/10">{{ $configuracoes['header_link_aniversariantes']->valor ?? 'Aniversariantes' }}</a>
                            <a href="{{ route('public.eventos.index') }}" class="text-white hover:text-blue-200 transition-colors font-medium py-2 px-3 rounded-lg hover:bg-white/10">{{ $configuracoes['header_link_eventos']->valor ?? 'Eventos' }}</a>
                            <a href="{{ route('doacao.index') }}" class="text-white hover:text-blue-200 transition-colors font-medium py-2 px-3 rounded-lg hover:bg-white/10">{{ $configuracoes['header_link_doacao']->valor ?? 'Doação' }}</a>
                            <a href="#contato" class="text-white hover:text-blue-200 transition-colors font-medium py-2 px-3 rounded-lg hover:bg-white/10">{{ $configuracoes['header_link_contato']->valor ?? 'Contato' }}</a>
                        @endif
                    @auth
                        <div class="pt-4 border-t border-white/20 mt-4">
                            ...
                            @if(auth()->user()->hasRole('Membro'))
                                <a href="{{ route('member.dashboard') }}" class="block w-full bg-white/10 text-white px-4 py-3 rounded-xl hover:bg-white hover:text-blue-600 transition-all duration-300 font-semibold text-center mb-3">
                                    <i class="fas fa-user mr-2"></i>{{ $configuracoes['header_texto_area_membro']->valor ?? 'Área do Membro' }}
                                </a>
                            @elseif(auth()->user()->hasAnyRole(['Super Admin', 'Pastor', 'Líder']))
                                <a href="{{ route('admin.dashboard') }}" class="block w-full bg-white/10 text-white px-4 py-3 rounded-xl hover:bg-white hover:text-blue-600 transition-all duration-300 font-semibold text-center mb-3">
                    ...
                    @else
                        <div class="pt-4 border-t border-white/20 mt-4">
                            <a href="{{ route('login') }}" class="block w-full bg-white/10 text-white px-4 py-3 rounded-xl hover:bg-white hover:text-blue-600 transition-all duration-300 font-semibold text-center">
                                <i class="fas fa-sign-in-alt mr-2"></i>{{ $configuracoes['header_texto_area_membro']->valor ?? 'Área do Membro' }}
                            </a>
                        </div>
                    @endauth
    ...
    <section class="relative py-20 hero-pattern" 
             style="@if($heroFotoFundoAtiva == '1' && $heroFotoFundo) 
                        background-image: url('{{ Storage::url($heroFotoFundo) }}'); 
        ...
        @if($heroFotoFundoAtiva == '1' && $heroFotoFundo)
            <div class="absolute inset-0" 
                 style="background-color: {{ $configuracoes['hero_overlay_cor']->valor ?? '#1e40af' }}; 
                        opacity: {{ $configuracoes['hero_overlay_opacidade']->valor ?? '0.6' }};"></div>
        @endif
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div class="text-center">
                <h1 class="text-5xl md:text-6xl font-bold text-white text-shadow mb-6">
                    {{ isset($configuracoes['hero_titulo']) && $configuracoes['hero_titulo']->valor ? $configuracoes['hero_titulo']->valor : 'Bem-vindo à Nossa Igreja' }}
                </h1>
                <p class="text-xl md:text-2xl text-white text-opacity-90 mb-8 max-w-3xl mx-auto">
                    {{ isset($configuracoes['hero_subtitulo']) && $configuracoes['hero_subtitulo']->valor ? $configuracoes['hero_subtitulo']->valor : 'Uma comunidade de fé, amor e esperança. Venha fazer parte da nossa família!' }}
                </p>
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    <a href="{{ $configuracoes['home_botao_link']->valor ?? '#sobre' }}" class="bg-white text-blue-600 px-8 py-4 rounded-xl font-semibold text-lg hover:bg-blue-50 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105">
                        <i class="fas fa-info-circle mr-2"></i>{{ $configuracoes['home_botao_texto']->valor ?? 'Conheça-nos' }}
                    </a>
                    @if(($configuracoes['doacao_ativa']->valor ?? '1') == '1')
                    <a href="{{ route('doacao.index') }}" class="glass-effect text-white px-8 py-4 rounded-xl font-semibold text-lg hover:bg-white hover:text-blue-600 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105">
                        <i class="fas fa-heart mr-2"></i>{{ $configuracoes['home_botao_doacao_texto']->valor ?? 'Faça uma Doação' }}
                    </a>
                    @endif
                </div>
            </div>
    ...
    <!-- Seção Sobre -->
    @if(($configuracoes['secao_sobre_ativa']->valor ?? '1') == '1')
    <section id="sobre" class="py-20 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-bold text-gray-900 mb-4">{{ $configuracoes['sobre_titulo']->valor ?? 'Sobre Nossa Igreja' }}</h2>
                <p class="text-xl text-gray-600 max-w-3xl mx-auto">
                    {{ $configuracoes['igreja_slogan']->valor ?? 'Uma igreja comprometida com o amor de Cristo e o serviço ao próximo.' }}
                </p>
            </div>
            ...
            <!-- Princípios Batistas -->
            @if(($configuracoes['principios_batistas_ativa']->valor ?? '1') == '1')
            <div class="mt-16 bg-gradient-to-r from-blue-600 to-blue-800 rounded-2xl p-8 text-white">
                <h3 class="text-2xl font-bold text-center mb-8">{{ $configuracoes['principios_batistas_titulo']->valor ?? 'Nossos Princípios Batistas' }}</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div class="text-center">
                        ...
                        <h4 class="font-semibold mb-2">{{ $configuracoes['principio_1_titulo']->valor ?? 'Sola Scriptura' }}</h4>
                        <p class="text-sm text-blue-100">{{ $configuracoes['principio_1_descricao']->valor ?? 'A Bíblia como única regra de fé e prática' }}</p>
                    </div>
                    ...
                    <div class="text-center">
                        ...
                        <h4 class="font-semibold mb-2">{{ $configuracoes['principio_2_titulo']->valor ?? 'Sacerdócio Universal' }}</h4>
                        <p class="text-sm text-blue-100">{{ $configuracoes['principio_2_descricao']->valor ?? 'Todo crente tem acesso direto a Deus' }}</p>
                    </div>
                    ...
                    <div class="text-center">
                        ...
                        <h4 class="font-semibold mb-2">{{ $configuracoes['principio_3_titulo']->valor ?? 'Batismo por Imersão' }}</h4>
                        <p class="text-sm text-blue-100">{{ $configuracoes['principio_3_descricao']->valor ?? 'Para crentes professos, por imersão' }}</p>
                    </div>
                    ...
                    <div class="text-center">
                        ...
                        <h4 class="font-semibold mb-2">{{ $configuracoes['principio_4_titulo']->valor ?? 'Autonomia Local' }}</h4>
                        <p class="text-sm text-blue-100">{{ $configuracoes['principio_4_descricao']->valor ?? 'Cada igreja é independente e autônoma' }}</p>
                    </div>
                </div>
            </div>
            @endif
    ...
    <!-- Seção Ministérios -->
    @if(($configuracoes['secao_ministerios_ativa']->valor ?? '1') == '1')
    <section id="ministerios" class="py-20 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-bold text-gray-900 mb-4">{{ $configuracoes['ministerios_titulo']->valor ?? 'Nossos Ministérios' }}</h2>
                <p class="text-xl text-gray-600 max-w-3xl mx-auto">
                    {{ $configuracoes['ministerios_subtitulo']->valor ?? 'Conheça nossos ministérios e participe da obra de Deus.' }}
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="card-hover bg-white rounded-2xl p-6 shadow-lg">
                    <div class="flex items-center mb-4">
                        <div class="w-12 h-12 rounded-full flex items-center justify-center mr-4" style="background-color: {{ $ministerio->cor }}">
                            <i class="fas fa-hands-helping text-white"></i>
                        </div>
                        <div>
                            <h3 class="text-lg font-semibold text-gray-900">{{ $ministerio->nome }}</h3>
                            <p class="text-sm text-gray-600">{{ $ministerio->membros_count ?? 0 }} membros</p>
                        </div>
                    </div>
                    <p class="text-gray-600 text-sm">{{ $ministerio->descricao ?? 'Ministério dedicado ao serviço de Deus.' }}</p>
                </div>
                @endforeach
            </div>
        </div>
    ...
    @if(($configuracoes['secao_cultos_ativa']->valor ?? '1') == '1')
    <section id="cultos" class="py-20 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-bold text-gray-900 mb-4">{{ $configuracoes['cultos_titulo']->valor ?? 'Horários de Cultos' }}</h2>
                <p class="text-xl text-gray-600 max-w-3xl mx-auto">
                    {{ $configuracoes['cultos_subtitulo']->valor ?? 'Venha adorar conosco e crescer na fé através da Palavra de Deus' }}
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div class="card-hover bg-gradient-to-br from-yellow-50 to-orange-50 rounded-2xl p-8 border border-yellow-100">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ $configuracoes['culto_domingo_manha_titulo']->valor ?? 'Culto de Domingo - Manhã' }}</h3>
                    <p class="text-2xl font-bold text-yellow-600 mb-4">{{ $configuracoes['culto_domingo_manha_horario']->valor ?? '09:00h' }}</p>
                    <p class="text-gray-600">{{ $configuracoes['culto_domingo_manha_descricao']->valor ?? 'Culto de adoração e pregação da Palavra de Deus' }}</p>
                    <div class="mt-4 text-sm text-gray-500">
                        <p><i class="fas fa-music mr-2"></i>{{ $configuracoes['culto_domingo_manha_item1']->valor ?? 'Louvor e Adoração' }}</p>
                        <p><i class="fas fa-bible mr-2"></i>{{ $configuracoes['culto_domingo_manha_item2']->valor ?? 'Pregação da Palavra' }}</p>
                        <p><i class="fas fa-pray mr-2"></i>{{ $configuracoes['culto_domingo_manha_item3']->valor ?? 'Oração e Intercessão' }}</p>
                    </div>
                </div>
                ...
                <div class="card-hover bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-8 border border-blue-100">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ $configuracoes['culto_domingo_noite_titulo']->valor ?? 'Culto de Domingo - Noite' }}</h3>
                    <p class="text-2xl font-bold text-blue-600 mb-4">{{ $configuracoes['culto_domingo_noite_horario']->valor ?? '18:00h' }}</p>
                    <p class="text-gray-600">{{ $configuracoes['culto_domingo_noite_descricao']->valor ?? 'Culto de celebração e edificação espiritual' }}</p>
                    <div class="mt-4 text-sm text-gray-500">
                        <p><i class="fas fa-music mr-2"></i>{{ $configuracoes['culto_domingo_noite_item1']->valor ?? 'Louvor e Adoração' }}</p>
                        <p><i class="fas fa-bible mr-2"></i>{{ $configuracoes['culto_domingo_noite_item2']->valor ?? 'Pregação da Palavra' }}</p>
                        <p><i class="fas fa-pray mr-2"></i>{{ $configuracoes['culto_domingo_noite_item3']->valor ?? 'Oração e Intercessão' }}</p>
                    </div>
                </div>
                ...
                <div class="card-hover bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-8 border border-green-100">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ $configuracoes['culto_quarta_titulo']->valor ?? 'Culto de Quarta-feira' }}</h3>
                    <p class="text-2xl font-bold text-green-600 mb-4">{{ $configuracoes['culto_quarta_horario']->valor ?? '19:30h' }}</p>
                    <p class="text-gray-600">{{ $configuracoes['culto_quarta_descricao']->valor ?? 'Culto de oração e estudo bíblico' }}</p>
                    <div class="mt-4 text-sm text-gray-500">
                        <p><i class="fas fa-pray mr-2"></i>{{ $configuracoes['culto_quarta_item1']->valor ?? 'Oração e Intercessão' }}</p>
                        <p><i class="fas fa-book-open mr-2"></i>{{ $configuracoes['culto_quarta_item2']->valor ?? 'Estudo Bíblico' }}</p>
                        <p><i class="fas fa-hands-helping mr-2"></i>{{ $configuracoes['culto_quarta_item3']->valor ?? 'Comunhão' }}</p>
                    </div>
                </div>
            </div>
            
            <!-- Escola Dominical -->
            @if(($configuracoes['escola_dominical_ativa']->valor ?? '1') == '1')
            <div class="mt-12 bg-gradient-to-r from-purple-600 to-purple-800 rounded-2xl p-8 text-white">
                <div class="text-center">
                    ...
                    <h3 class="text-2xl font-bold mb-4">{{ $configuracoes['escola_dominical_titulo']->valor ?? 'Escola Dominical' }}</h3>
                    <p class="text-xl mb-6">{{ $configuracoes['escola_dominical_horario']->valor ?? 'Domingo às 08:00h' }}</p>
                    <p class="text-purple-100 max-w-2xl mx-auto">
                        {{ $configuracoes['escola_dominical_descricao']->valor ?? 'Venha estudar a Bíblia conosco! A Escola Dominical é um momento especial para aprender mais sobre a Palavra de Deus, crescer na fé e fortalecer nossa comunhão.' }}
                    </p>
                    <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div class="text-center">
                            <i class="fas fa-child text-2xl mb-2"></i>
                            <p>{{ $configuracoes['escola_dominical_classe1']->valor ?? 'Classes Infantis' }}</p>
                        </div>
                        <div class="text-center">
                            <i class="fas fa-users text-2xl mb-2"></i>
                            <p>{{ $configuracoes['escola_dominical_classe2']->valor ?? 'Classes de Jovens' }}</p>
                        </div>
                        <div class="text-center">
                            <i class="fas fa-user-friends text-2xl mb-2"></i>
                            <p>{{ $configuracoes['escola_dominical_classe3']->valor ?? 'Classes de Adultos' }}</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <!-- Seção Eventos -->
    @if(($configuracoes['secao_eventos_ativa']->valor ?? '1') == '1')
    <section id="eventos" class="py-20 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse(\App\Models\Evento::where('ativo', true)->where('destaque', true)->where('data_inicio', '>=', now())->orderBy('data_inicio')->limit(6)->get() as $evento)
                <div class="card-hover bg-white rounded-2xl shadow-lg overflow-hidden">
                    @if($evento->imagem)
                        <img src="{{ Storage::url($evento->imagem) }}" alt="{{ $evento->titulo }}" class="w-full h-48 object-cover">
                    @else
                    ...
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium 
                                ...
                                {{ $evento->gratuito ? 'Gratuito' : 'R$ ' . number_format($evento->valor_inscricao, 2, ',', '.') }}
                            </span>
                        </div>
    ...
    @if(($configuracoes['secao_aniversariantes_ativa']->valor ?? '1') == '1' && ($configuracoes['aniversariantes_mostrar']->valor ?? '1') == '1')
    <section id="aniversariantes" class="py-20 bg-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-bold text-gray-900 mb-4">{{ $configuracoes['aniversariantes_titulo']->valor ?? 'Aniversariantes do Mês' }}</h2>
                <p class="text-xl text-gray-600 max-w-3xl mx-auto">
                    {{ $configuracoes['aniversariantes_subtitulo']->valor ?? 'Celebrando a vida dos nossos membros' }}
                </p>
            </div>
                ...
                <div class="card-hover bg-gradient-to-br from-pink-50 to-rose-50 rounded-2xl p-6 border border-pink-100">
                    <div class="flex items-center mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">{{ $aniversariante->data_nascimento->format('d/m') }}</p>
                        </div>
                    </div>
    ...
    <section id="doacao" class="py-20 bg-gradient-to-br from-green-600 to-emerald-700">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-bold text-white mb-4">{{ $configuracoes['doacao_titulo']->valor ?? 'Faça uma Doação' }}</h2>
                <p class="text-xl text-green-100 max-w-3xl mx-auto">
                    {{ $configuracoes['doacao_subtitulo']->valor ?? 'Sua doação ajuda a manter nossa igreja e a expandir a obra de Deus. Seja uma bênção!' }}
                </p>
            </div>
            
            <!-- Seção de Doação Profissional -->
            @if(in_array($doacao_sem_login, [1, '1', true, 'true']) && in_array($doacao_ativa, [1, '1', true, 'true']))
            <div class="bg-gradient-to-br from-green-50 to-blue-50 rounded-3xl shadow-2xl p-8 mb-8 border border-green-100">
                ...
                <div class="text-center mb-8">
                    ...
                    <p class="text-lg text-gray-600">{{ $configuracoes['doacao_titulo']->valor ?? 'Contribua para a obra de Deus e seja uma bênção para nossa comunidade' }}</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div class="bg-white rounded-xl p-6 text-center shadow-lg">
                        ...
                        <p class="text-sm text-gray-600">{{ $configuracoes['doacao_dica_seguranca']->valor ?? 'Seus dados estão protegidos com criptografia SSL' }}</p>
                    </div>
                    ...
                    <div class="bg-white rounded-xl p-6 text-center shadow-lg">
                        ...
                        <p class="text-sm text-gray-600">{{ $configuracoes['doacao_dica_comprovante']->valor ?? 'Receba um comprovante por email após a confirmação' }}</p>
                    </div>
                    ...
                    <div class="bg-white rounded-xl p-6 text-center shadow-lg">
                        ...
                        <p class="text-sm text-gray-600">{{ $configuracoes['doacao_dica_transparencia']->valor ?? 'Todas as doações são registradas e auditadas' }}</p>
                    </div>
                </div>

                <!-- Formulário de Doação -->
                <div class="bg-white rounded-2xl shadow-lg p-8">
                    <form method="POST" action="{{ route('doacao.process') }}" class="max-w-lg mx-auto">
                        @csrf
                        <div class="space-y-6">
                            <!-- Valor da Doação -->
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" 
                                           name="valor" 
                                           step="0.01" 
                                           min="{{ $configuracoes['doacao_valor_minimo']->valor ?? '1' }}" 
                                           max="{{ $configuracoes['doacao_valor_maximo']->valor ?? '10000' }}"
                                ...
                                <p class="text-sm text-gray-500 mt-2">
                                    Valor mínimo: R$ {{ number_format($configuracoes['doacao_valor_minimo']->valor ?? 1, 2, ',', '.') }} | 
                                    Valor máximo: R$ {{ number_format($configuracoes['doacao_valor_maximo']->valor ?? 10000, 2, ',', '.') }}
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    ...
                                    @if(isset($campanhas) && $campanhas->count() > 0)
                                    <label class="relative cursor-pointer">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    @if(!empty($configuracoes['stripe_key']->valor ?? ''))
                                    <label class="relative cursor-pointer">
                                    ...
                                    @if(!empty($configuracoes['mercadopago_key']->valor ?? ''))
                                    <label class="relative cursor-pointer">
                                    ...
                                    @if(empty($configuracoes['stripe_key']->valor ?? '') && empty($configuracoes['mercadopago_key']->valor ?? ''))
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div class="text-center card-hover bg-white rounded-2xl p-8">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ $configuracoes['doacao_dizimo_titulo']->valor ?? 'Dízimo' }}</h3>
                    <p class="text-gray-600 mb-6">{{ $configuracoes['doacao_dizimo_descricao']->valor ?? 'Contribua com o dízimo para a manutenção da igreja.' }}</p>
                    <a href="{{ route('doacao.index') }}" class="bg-green-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-green-700 transition-all duration-300">
                        {{ $configuracoes['doacao_dizimo_botao']->valor ?? 'Doar Dízimo' }}
                    </a>
                </div>
                ...
                <div class="text-center card-hover bg-white rounded-2xl p-8">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ $configuracoes['doacao_oferta_titulo']->valor ?? 'Oferta' }}</h3>
                    <p class="text-gray-600 mb-6">{{ $configuracoes['doacao_oferta_descricao']->valor ?? 'Ofereça com amor para as necessidades da igreja.' }}</p>
                    <a href="{{ route('doacao.index') }}" class="bg-blue-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-blue-700 transition-all duration-300">
                        {{ $configuracoes['doacao_oferta_botao']->valor ?? 'Fazer Oferta' }}
                    </a>
                </div>
                ...
                <div class="text-center card-hover bg-white rounded-2xl p-8">
                    ...
                    <h3 class="text-xl font-semibold text-gray-900 mb-4">{{ $configuracoes['doacao_campanhas_titulo']->valor ?? 'Campanhas' }}</h3>
                    <p class="text-gray-600 mb-6">{{ $configuracoes['doacao_campanhas_descricao']->valor ?? 'Participe de nossas campanhas especiais.' }}</p>
                    <a href="{{ route('doacao.index') }}" class="bg-purple-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-purple-700 transition-all duration-300">
                        {{ $configuracoes['doacao_campanhas_botao']->valor ?? 'Ver Campanhas' }}
                    </a>
                </div>
            </div>
        </div>
    ...
    <!-- Seção Contato -->
    @if(($configuracoes['secao_contato_ativa']->valor ?? '1') == '1')
    <section id="contato" class="py-20 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-4xl font-bold text-gray-900 mb-4">{{ $configuracoes['contato_titulo']->valor ?? 'Entre em Contato' }}</h2>
                <p class="text-xl text-gray-600 max-w-3xl mx-auto">
                    {{ $configuracoes['contato_subtitulo']->valor ?? 'Estamos aqui para você. Entre em contato conosco!' }}
                </p>
            </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-gray-600">{{ $configuracoes['igreja_endereco']->valor ?? 'Endereço da igreja' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-gray-600">{{ $configuracoes['igreja_telefone']->valor ?? '(11) 99999-9999' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-gray-600">{{ $configuracoes['igreja_email']->valor ?? 'contato@igreja.com' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center">
                            ...
                            <div>
                                ...
                                <p class="text-gray-600">{{ $configuracoes['culto_domingo']->valor ?? 'Domingo: 09:00 e 18:00' }}</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <!-- Footer Profissional -->
    @include('components.footer-welcome')
            ...
            <div class="flex items-start space-x-3">
                ...
                <div>
                    ...
                    <p class="text-gray-300 text-sm leading-relaxed">
                        ...
                        <a href="{{ route('politica-cookies') }}" class="text-blue-400 hover:text-blue-300 underline">Política de Cookies</a> e 
                        <a href="{{ route('politica-privacidade') }}" class="text-blue-400 hover:text-blue-300 underline">Política de Privacidade</a>.
                    </p>
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl max-w-2xl w-full max-h-96 overflow-y-auto">
            <div class="p-6">
                ...
                <div class="mt-4 text-center">
                    <a href="{{ route('politica-cookies') }}" class="text-blue-600 hover:text-blue-700 text-sm underline">
