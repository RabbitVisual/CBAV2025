...
<head>
    ...
    <title>Validação de Documento - CBAV Sistema</title>
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div class="mt-4 bg-yellow-50 border border-yellow-200 rounded-lg p-6">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 flex-1">
                            <h4 class="text-lg font-medium text-yellow-800">⚠️ Documento Vencido</h4>
                            <div class="mt-2 text-sm text-yellow-700">
                                ...
                                <div class="mt-4 p-4 bg-yellow-100 rounded border-l-4 border-yellow-400">
                                    ...
                                    <ul class="text-sm text-yellow-700 space-y-2">
                                        ...
                                        <li>• <strong>Limitações:</strong> Não é válido para declarações atuais</li>
                                    </ul>
                                </div>
                                ...
                                <div class="mt-4 p-4 bg-blue-50 rounded border-l-4 border-blue-400">
                                    ...
                                    <p class="text-sm text-blue-700">
                                        Este documento possui <strong>hash de validação válido</strong>, confirmando que não foi adulterado. 
                                        O vencimento não afeta a autenticidade, mas pode limitar seu uso para fins atuais. 
                                        Para declarações da Receita Federal, consulte documentos dentro do prazo de validade.
                                    </p>
                                </div>
                                ...
                                <div class="mt-4 p-4 bg-red-50 rounded border-l-4 border-red-400">
                                    ...
                                    <p class="text-sm text-red-700">
                                        <strong>Importante:</strong> Documentos vencidos podem ser usados por fraudadores para criar documentos falsos. 
                                        Sempre verifique a data de vencimento e use apenas documentos atuais para declarações oficiais.
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            @if($valido && !$documento->isVencido())
                <div class="mt-4 bg-green-50 border border-green-200 rounded-lg p-6">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3 flex-1">
                            <h4 class="text-lg font-medium text-green-800">✅ Documento Válido e Atual</h4>
                            <div class="mt-2 text-sm text-green-700">
                                ...
                                <div class="mt-4 p-4 bg-green-100 rounded border-l-4 border-green-400">
                                    ...
                                    <ul class="text-sm text-green-700 space-y-2">
                                        ...
                                        <li>• <strong>Apto para Uso:</strong> Pode ser usado para declarações</li>
                                ...
                                <div class="mt-4 p-4 bg-blue-50 rounded border-l-4 border-blue-400">
                                    ...
                                    <p class="text-sm text-blue-700">
                                        Este documento passou por todas as verificações de segurança e está apto para uso oficial. 
        ...
        <div class="bg-white shadow rounded-lg">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-gray-900">Documento de Declaração Anual</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <dl class="space-y-2">
                            ...
                            <div>
                                ...
                                <dd class="text-sm text-gray-900">{{ \App\Models\DocumentoDeclaracaoAnual::TIPOS_DOCUMENTO[$documento->tipo_documento] ?? $documento->tipo_documento }}</dd>
                            </div>
                            <div>
                    ...
                    <div>
                        ...
                        <dl class="space-y-2">
                            ...
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Validado em</dt>
                                <dd class="text-sm text-gray-900">{{ $documento->validado_em->format('d/m/Y H:i:s') }}</dd>
                            </div>
                            @endif
                            @if($documento->validadoPor)
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Validado por</dt>
                                <dd class="text-sm text-gray-900">{{ $documento->validadoPor->name }}</dd>
                            </div>
                            @endif
                        </dl>
                    </div>
            ...
            <div class="px-6 py-4 border-t border-gray-200">
                <h3 class="text-sm font-medium text-gray-900 mb-3">Informações da Entidade Religiosa</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <dl class="space-y-2">
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Nome</dt>
                                <dd class="text-sm text-gray-900">{{ $documento->igreja->nome ?? 'N/A' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium text-gray-500">CNPJ</dt>
                                <dd class="text-sm text-gray-900 font-mono">{{ $documento->igreja->cnpj_formatado ?? 'N/A' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Endereço</dt>
                                <dd class="text-sm text-gray-900">{{ $documento->igreja->endereco_completo ?? 'N/A' }}</dd>
                            </div>
                        </dl>
                    </div>
                    <div>
                        <dl class="space-y-2">
                            <div>
                                ...
                                <dd class="text-sm text-gray-900">{{ $documento->igreja->telefone ?? 'N/A' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Email</dt>
                                <dd class="text-sm text-gray-900">{{ $documento->igreja->email ?? 'N/A' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Pastor Responsável</dt>
                                <dd class="text-sm text-gray-900">{{ $documento->igreja->pastor_responsavel ?? 'N/A' }}</dd>
                            </div>
                        </dl>
                    </div>
                </div>
            ...
            <!-- Valores Financeiros -->
            <div class="px-6 py-4 border-t border-gray-200">
                <h3 class="text-sm font-medium text-gray-900 mb-3">Valores Financeiros</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        <dt class="text-xs font-medium text-gray-500">Valor Total</dt>
                        <dd class="text-lg font-semibold text-gray-900">R$ {{ number_format($documento->valor_total, 2, ',', '.') }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium text-gray-500">Doações</dt>
                        <dd class="text-sm text-gray-900">R$ {{ number_format($documento->valor_doacoes, 2, ',', '.') }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium text-gray-500">Dízimos</dt>
                        <dd class="text-sm text-gray-900">R$ {{ number_format($documento->valor_dizimos, 2, ',', '.') }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium text-gray-500">Outros</dt>
                        <dd class="text-sm text-gray-900">R$ {{ number_format($documento->valor_outros, 2, ',', '.') }}</dd>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <dl class="space-y-2">
                            ...
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Certificado Digital</dt>
                                <dd class="text-xs text-gray-900 font-mono break-all">{{ $documento->certificado_digital }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Assinatura Digital</dt>
                                <dd class="text-xs text-gray-900 font-mono break-all">{{ $documento->assinatura_digital }}</dd>
                            </div>
                        </dl>
                    </div>
                    <div>
                        <dl class="space-y-2">
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Código de Barras</dt>
                                <dd class="text-xs text-gray-900 font-mono break-all">{{ $documento->codigo_barras }}</dd>
                            </div>
                            @if($multaJuros['dias_vencido'] > 0)
                            <div>
                                <dt class="text-xs font-medium text-gray-500">Multa e Juros</dt>
                                <dd class="text-sm text-red-600 font-semibold">R$ {{ number_format($multaJuros['multa'] + $multaJuros['juros'], 2, ',', '.') }}</dd>
                            </div>
                            @endif
                        </dl>
                    </div>
            ...
            <div class="px-6 py-4 border-t border-gray-200">
                ...
                <div class="text-center mb-6">
                    <h4 class="text-sm font-medium text-gray-900 mb-3">QR Code para Validação</h4>
                    <div class="inline-block p-4 bg-white border rounded-lg">
                        {!! $documento->gerarQRCodeSVG() !!}
                    </div>
                    <p class="text-xs text-gray-600 mt-2">Escaneie para validar a autenticidade</p>
                </div>

                <!-- Código de Barras -->
                <div class="text-center">
                    <h4 class="text-sm font-medium text-gray-900 mb-3">Código de Barras para Validação</h4>
                    <div class="inline-block p-3 bg-white border rounded-lg">
                        <div style="transform: scale(0.6); transform-origin: center;">
                            {!! $documento->gerarCodigoBarrasSVG() !!}
                        </div>
                    </div>
                    <p class="text-xs text-gray-600 mt-2">Utilize para validação na Receita Federal</p>
                </div>
            </div>
