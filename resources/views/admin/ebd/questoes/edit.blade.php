...
@section('title', 'Editar Questão EBD')
...
<div class="container mx-auto px-4 py-8">
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 mb-2">Editar Questão EBD</h1>
                <p class="text-gray-600">Edite a questão para avaliações da Escola Bíblica Dominical</p>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('admin.ebd.questoes.update', $questao) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <select name="avaliacao_id" id="avaliacao_id" required 
                        ...
                        @foreach($avaliacoes as $avaliacao)
                            <option value="{{ $avaliacao->id }}" 
                                {{ old('avaliacao_id', $questao->avaliacao_id) == $avaliacao->id ? 'selected' : '' }}>
                                {{ $avaliacao->titulo }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="pergunta" id="pergunta" rows="4" required
                              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Digite a pergunta da questão...">{{ old('pergunta', $questao->pergunta) }}</textarea>
                    @error('pergunta')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="tipo" id="tipo" required
                        ...
                        @foreach($tipos as $valor => $nome)
                            <option value="{{ $valor }}" {{ old('tipo', $questao->tipo) == $valor ? 'selected' : '' }}>{{ $nome }}</option>
                        @endforeach
                    </select>
                    @error('tipo')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="dificuldade" id="dificuldade" required
                        ...
                        <option value="facil" {{ old('dificuldade', $questao->dificuldade ?? '') == 'facil' ? 'selected' : '' }}>Fácil</option>
                        <option value="medio" {{ old('dificuldade', $questao->dificuldade ?? '') == 'medio' ? 'selected' : '' }}>Médio</option>
                        <option value="dificil" {{ old('dificuldade', $questao->dificuldade ?? '') == 'dificil' ? 'selected' : '' }}>Difícil</option>
                    </select>
                    @error('dificuldade')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="pontuacao" id="pontuacao" value="{{ old('pontuacao', $questao->pontuacao) }}" 
                ...
                <div>
                    ...
                    <input type="text" name="resposta_correta" id="resposta_correta" value="{{ old('resposta_correta', $questao->resposta_correta) }}"
                           class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div id="opcoes-container" class="md:col-span-2 {{ $questao->tipo === 'multipla_escolha' ? '' : 'hidden' }}">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Opções de Resposta</label>
                    <div id="opcoes-list" class="space-y-2">
                        @if($questao->opcoes && is_array($questao->opcoes))
                            @foreach($questao->opcoes as $index => $opcao)
                                <div class="flex items-center space-x-2">
                                    <input type="text" name="opcoes[]" value="{{ old('opcoes.' . $index, $opcao) }}"
                                           class="flex-1 border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                           placeholder="Opção {{ chr(65 + $index) }}">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="explicacao" id="explicacao" rows="3"
                              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Explicação detalhada da resposta correta...">{{ old('explicacao', $questao->explicacao) }}</textarea>
                    @error('explicacao')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    <div class="flex items-center">
                        <input type="checkbox" name="ativo" id="ativo" value="1" 
                               {{ old('ativo', $questao->ativo ?? true) ? 'checked' : '' }}
...
function addOpcao() {
    ...
    const currentCount = opcoesList.children.length;
    const newOpcao = document.createElement('div');
    newOpcao.className = 'flex items-center space-x-2';
    newOpcao.innerHTML = `
        <input type="text" name="opcoes[]" 
               class="flex-1 border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
               placeholder="Opção ${String.fromCharCode(65 + currentCount)}">
        <button type="button" onclick="removeOpcao(this)" 
                class="px-3 py-2 text-red-600 hover:text-red-800">
            <i class="fas fa-trash"></i>
        </button>
