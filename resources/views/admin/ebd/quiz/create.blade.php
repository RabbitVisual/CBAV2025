...
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Nova Pergunta</h1>
                <p class="text-gray-600">Crie uma nova pergunta para o quiz bíblico</p>
            </div>
    ...
    <div class="bg-white rounded-lg shadow">
        <form action="{{ route('admin.ebd.quiz-biblico.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <textarea id="pergunta" name="pergunta" rows="3" required
                          ...
                          placeholder="Digite a pergunta...">{{ old('pergunta') }}</textarea>
                @error('pergunta')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="opcao_a" name="opcao_a" required
                           ...
                           value="{{ old('opcao_a') }}" placeholder="Digite a opção A">
                    @error('opcao_a')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="opcao_b" name="opcao_b" required
                           ...
                           value="{{ old('opcao_b') }}" placeholder="Digite a opção B">
                    @error('opcao_b')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="opcao_c" name="opcao_c" required
                           ...
                           value="{{ old('opcao_c') }}" placeholder="Digite a opção C">
                    @error('opcao_c')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="opcao_d" name="opcao_d" required
                           ...
                           value="{{ old('opcao_d') }}" placeholder="Digite a opção D">
                    @error('opcao_d')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <select id="resposta_correta" name="resposta_correta" required
                    ...
                    <option value="a" {{ old('resposta_correta') == 'a' ? 'selected' : '' }}>Opção A</option>
                    <option value="b" {{ old('resposta_correta') == 'b' ? 'selected' : '' }}>Opção B</option>
                    <option value="c" {{ old('resposta_correta') == 'c' ? 'selected' : '' }}>Opção C</option>
                    <option value="d" {{ old('resposta_correta') == 'd' ? 'selected' : '' }}>Opção D</option>
                </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select id="categoria" name="categoria" required
                        ...
                        <option value="antigo_testamento" {{ old('categoria') == 'antigo_testamento' ? 'selected' : '' }}>Antigo Testamento</option>
                        <option value="novo_testamento" {{ old('categoria') == 'novo_testamento' ? 'selected' : '' }}>Novo Testamento</option>
                        <option value="personagens" {{ old('categoria') == 'personagens' ? 'selected' : '' }}>Personagens</option>
                        <option value="milagres" {{ old('categoria') == 'milagres' ? 'selected' : '' }}>Milagres</option>
                        <option value="parabolas" {{ old('categoria') == 'parabolas' ? 'selected' : '' }}>Parábolas</option>
                        <option value="profetas" {{ old('categoria') == 'profetas' ? 'selected' : '' }}>Profetas</option>
                        <option value="apostolos" {{ old('categoria') == 'apostolos' ? 'selected' : '' }}>Apóstolos</option>
                    </select>
                    @error('categoria')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" id="pontuacao" name="pontuacao" min="1" max="100" required
                           ...
                           value="{{ old('pontuacao', 10) }}" placeholder="10">
                    @error('pontuacao')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div>
                ...
                <input type="text" id="referencia_biblica" name="referencia_biblica"
                       ...
                       value="{{ old('referencia_biblica') }}" placeholder="Ex: João 3:16">
            ...
            <div>
                ...
                <textarea id="explicacao" name="explicacao" rows="4"
                          ...
                          placeholder="Digite uma explicação para a resposta correta...">{{ old('explicacao') }}</textarea>
                @error('explicacao')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
