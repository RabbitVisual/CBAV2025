...
@section('title', 'Editar Certificado EBD')
...
<div class="container mx-auto px-4 py-8">
    <div class="mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 mb-2">Editar Certificado EBD</h1>
                <p class="text-gray-600">Edite o certificado da Escola Bíblica Dominical</p>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('admin.ebd.certificados.update', $certificado) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <select name="aluno_id" id="aluno_id" required 
                        ...
                        @foreach($alunos as $aluno)
                            <option value="{{ $aluno->id }}" 
                                {{ old('aluno_id', $certificado->aluno_id) == $aluno->id ? 'selected' : '' }}>
                    ...
                    <select name="avaliacao_id" id="avaliacao_id"
                        ...
                        @foreach($avaliacoes as $avaliacao)
                            <option value="{{ $avaliacao->id }}" 
                                {{ old('avaliacao_id', $certificado->avaliacao_id) == $avaliacao->id ? 'selected' : '' }}>
                                {{ $avaliacao->titulo }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="titulo" id="titulo" value="{{ old('titulo', $certificado->titulo) }}" required
                           class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                    ...
                    <select name="tipo" id="tipo" required
                        ...
                        @foreach($tipos as $valor => $nome)
                            <option value="{{ $valor }}" 
                                {{ old('tipo', $certificado->tipo) == $valor ? 'selected' : '' }}>{{ $nome }}</option>
                        @endforeach
                    </select>
                    @error('tipo')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="date" name="data_emissao" id="data_emissao" 
                           value="{{ old('data_emissao', $certificado->data_emissao->format('Y-m-d')) }}" required
                           class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @error('data_emissao')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="carga_horaria" id="carga_horaria" 
                           value="{{ old('carga_horaria', $certificado->carga_horaria) }}" 
                           min="1" max="1000"
                           class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                           placeholder="Ex: 40">
                    @error('carga_horaria')
                ...
                <div>
                    ...
                    <input type="number" name="nota_final" id="nota_final" 
                           value="{{ old('nota_final', $certificado->nota_final) }}" 
                ...
                <div>
                    ...
                    <input type="date" name="data_validade" id="data_validade" 
                           value="{{ old('data_validade', $certificado->data_validade ? $certificado->data_validade->format('Y-m-d') : '') }}"
                           class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @error('data_validade')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="descricao" id="descricao" rows="3"
                              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Descrição detalhada do certificado...">{{ old('descricao', $certificado->descricao) }}</textarea>
                    @error('descricao')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="conteudo" id="conteudo" rows="8" required
                              class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Conteúdo detalhado que aparecerá no certificado...">{{ old('conteudo', $certificado->conteudo) }}</textarea>
                    @error('conteudo')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="assinatura_coordenador" id="assinatura_coordenador" 
                           value="{{ old('assinatura_coordenador', $certificado->assinatura_coordenador) }}"
                           class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                    ...
                    <input type="text" name="assinatura_pastor" id="assinatura_pastor" 
                           value="{{ old('assinatura_pastor', $certificado->assinatura_pastor) }}"
                           class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div class="md:col-span-2">
                    <div class="flex items-center">
                        <input type="checkbox" name="ativo" id="ativo" value="1" 
                               {{ old('ativo', $certificado->ativo) ? 'checked' : '' }}
