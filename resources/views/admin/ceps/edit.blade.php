        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md">
            ...
            <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between">
                    <div>
                        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar CEP</h1>
                        <p class="text-gray-600 dark:text-gray-400 mt-1">{{ $cep->localidade }} - {{ $cep->uf }}</p>
                    </div>
            ...
            <!-- Formulário -->
            <form action="{{ route('admin.people.ceps.update', $cep) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="cep_inicial" 
                               name="cep_inicial" 
                               value="{{ old('cep_inicial', $cep->cep_inicial) }}"
                               maxlength="8"
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="cep_final" 
                               name="cep_final" 
                               value="{{ old('cep_final', $cep->cep_final) }}"
                               maxlength="8"
                    ...
                    <div>
                        ...
                        <select id="uf" 
                            ...
                            <option value="AC" {{ old('uf', $cep->uf) == 'AC' ? 'selected' : '' }}>AC - Acre</option>
                            <option value="AL" {{ old('uf', $cep->uf) == 'AL' ? 'selected' : '' }}>AL - Alagoas</option>
                            <option value="AP" {{ old('uf', $cep->uf) == 'AP' ? 'selected' : '' }}>AP - Amapá</option>
                            <option value="AM" {{ old('uf', $cep->uf) == 'AM' ? 'selected' : '' }}>AM - Amazonas</option>
                            <option value="BA" {{ old('uf', $cep->uf) == 'BA' ? 'selected' : '' }}>BA - Bahia</option>
                            <option value="CE" {{ old('uf', $cep->uf) == 'CE' ? 'selected' : '' }}>CE - Ceará</option>
                            <option value="DF" {{ old('uf', $cep->uf) == 'DF' ? 'selected' : '' }}>DF - Distrito Federal</option>
                            <option value="ES" {{ old('uf', $cep->uf) == 'ES' ? 'selected' : '' }}>ES - Espírito Santo</option>
                            <option value="GO" {{ old('uf', $cep->uf) == 'GO' ? 'selected' : '' }}>GO - Goiás</option>
                            <option value="MA" {{ old('uf', $cep->uf) == 'MA' ? 'selected' : '' }}>MA - Maranhão</option>
                            <option value="MT" {{ old('uf', $cep->uf) == 'MT' ? 'selected' : '' }}>MT - Mato Grosso</option>
                            <option value="MS" {{ old('uf', $cep->uf) == 'MS' ? 'selected' : '' }}>MS - Mato Grosso do Sul</option>
                            <option value="MG" {{ old('uf', $cep->uf) == 'MG' ? 'selected' : '' }}>MG - Minas Gerais</option>
                            <option value="PA" {{ old('uf', $cep->uf) == 'PA' ? 'selected' : '' }}>PA - Pará</option>
                            <option value="PB" {{ old('uf', $cep->uf) == 'PB' ? 'selected' : '' }}>PB - Paraíba</option>
                            <option value="PR" {{ old('uf', $cep->uf) == 'PR' ? 'selected' : '' }}>PR - Paraná</option>
                            <option value="PE" {{ old('uf', $cep->uf) == 'PE' ? 'selected' : '' }}>PE - Pernambuco</option>
                            <option value="PI" {{ old('uf', $cep->uf) == 'PI' ? 'selected' : '' }}>PI - Piauí</option>
                            <option value="RJ" {{ old('uf', $cep->uf) == 'RJ' ? 'selected' : '' }}>RJ - Rio de Janeiro</option>
                            <option value="RN" {{ old('uf', $cep->uf) == 'RN' ? 'selected' : '' }}>RN - Rio Grande do Norte</option>
                            <option value="RS" {{ old('uf', $cep->uf) == 'RS' ? 'selected' : '' }}>RS - Rio Grande do Sul</option>
                            <option value="RO" {{ old('uf', $cep->uf) == 'RO' ? 'selected' : '' }}>RO - Rondônia</option>
                            <option value="RR" {{ old('uf', $cep->uf) == 'RR' ? 'selected' : '' }}>RR - Roraima</option>
                            <option value="SC" {{ old('uf', $cep->uf) == 'SC' ? 'selected' : '' }}>SC - Santa Catarina</option>
                            <option value="SP" {{ old('uf', $cep->uf) == 'SP' ? 'selected' : '' }}>SP - São Paulo</option>
                            <option value="SE" {{ old('uf', $cep->uf) == 'SE' ? 'selected' : '' }}>SE - Sergipe</option>
                            <option value="TO" {{ old('uf', $cep->uf) == 'TO' ? 'selected' : '' }}>TO - Tocantins</option>
                        </select>
                        @error('uf')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" 
                               id="faixa_de_cep" 
                               name="faixa_de_cep" 
                               value="{{ old('faixa_de_cep', $cep->faixa_de_cep) }}"
                    ...
                    <div>
                        ...
                        <input type="text" 
                               id="cod_ibge" 
                               name="cod_ibge" 
                               value="{{ old('cod_ibge', $cep->cod_ibge) }}"
                    ...
                    <div>
                        ...
                        <input type="number" 
                               id="latitude" 
                               name="latitude" 
                               value="{{ old('latitude', $cep->latitude) }}"
                               step="any"
                               min="-90"
                               max="90"
                               placeholder="-23.5505"
                    ...
                    <div>
                        ...
                        <input type="number" 
                               id="longitude" 
                               name="longitude" 
                               value="{{ old('longitude', $cep->longitude) }}"
                               step="any"
                               min="-180"
                               max="180"
                               placeholder="-46.6333"
